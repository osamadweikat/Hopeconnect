...
const fs = require("fs");
const { User, Volunteer, Orphanage, Donation, Payment, Sponsorship } = require("../models/index.js");


const generateReport = async () => {
    return new Promise(async (resolve, reject) => {
        const filePath = `./reports/admin_report_${Date.now()}.pdf`;
        ...
        const users = await User.findAll({ attributes: ["id", "full_name", "email", "role", "created_at"] });
        ...
        donations.forEach((donation) => {
            doc.fontSize(12).text(`- Donor ID: ${donation.donor_id}, Amount: $${donation.amount}, Category: ${donation.category}, Date: ${donation.created_at}`);
        });
        doc.moveDown(2);

        doc.fontSize(16).text("Orphanages", { underline: true });
        orphanages.forEach((orphanage) => {
            doc.fontSize(12).text(`- ${orphanage.name}, Location: ${orphanage.location}, Manager ID: ${orphanage.manager_id}`);
        });
        doc.moveDown(2);

        doc.fontSize(16).text("Sponsorships", { underline: true });
        sponsorships.forEach((sponsorship) => {
            doc.fontSize(12).text(`- Sponsor ID: ${sponsorship.sponsor_id}, Orphan ID: ${sponsorship.orphan_id}, Amount: $${sponsorship.amount}, Start: ${sponsorship.start_date}`);
        });
